...
@Autonomous(name = "FarAutoBetterVision")
...
public class FarAutoBetterVision extends OpMode {
    ...
    public enum PathState {
        ...
        DETECT, ONE_MORE_TIME, TEST, TWO_MORE_TIME
    }
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case ONE_MORE_TIME:
                ...
                //if(waitSecs(0.01)) {
                if (!detectInitDone && follower.getPose().getX() > 110) {
                    ArrayList<Object[]> detections = limelight.updateBall2();
                    ArrayList<Double> results2 = new ArrayList<>();
                    for(int col = 0; col < detections.get(0).length; col++)
                    {
                ...
                if(!follower.isBusy() && !detectPathSet && detectInitDone){
                    ...
                    if (posCount == 0 && negCount == 0 /* || alliance == OLDChoose.Alliance.BLUE*/) {
                        if(spikeMark == 3 || spikeMark == 5){
                            follower.followPath(paths.shootTo3NotSet(), 0.9, true);
                        } else {
                            follower.followPath(paths.shootTo4NotSet(), 0.9, true);
                        }
                    } else {
                        newY = paths.shootPose2.getY() + average;
                        if (newY < 9) { newY = 9; }
                        ...
                        if (newY < 12){ follower.followPath(paths.shootTo4NotSet(), 0.9, true); }
                        else { follower.followPath(paths.fromTo(ballCollect), 0.9, true); }
                    }
                }
            ...
            case TWO_MORE_TIME:
                ...
                //if(waitSecs(0.01)) {
                if (!detectInitDone && follower.getPose().getX() > 120) {
                    ArrayList<Object[]> detections = limelight.updateBall2();
                    ArrayList<Double> results2 = new ArrayList<>();
                    for(int col = 0; col < detections.get(0).length; col++)
                    {
